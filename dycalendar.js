(function (global) {
    ...
    function createMonthTable(data, option) {
        ...
        while (c <= 6) {
            ...
            const fullDate = `${data.year}-${data.monthIndex + 1}-${count}`;
            td.setAttribute("data-date", fullDate);
    ...
    dycalendar.draw = function (option) {
        ...
        global.currentCalendarOption = option; // ✅ SET GLOBAL VALUE HERE
        drawCalendar(option);
